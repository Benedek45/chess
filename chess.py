...
def main():
    ...
    white_points, black_points = points(board)
...
# 0. step
def chess_game_continue_or_start(): #hiba rossz beirasakor
...
def display_chessboard_graphical(board, root=None, canvas=None):
    if root is None:
        ...
        canvas = tk.Canvas(root, width=8 * 60 + 40, height=8 * 60 + 80)  # Extra space for labels and points
        canvas.pack()
    else:
    ...
    # Display points for both players at the bottom
    canvas.create_text(160, 550, text=f"         White Points: {white_points}  |  Black Points: {black_points}", font=("Arial", 16), fill="black")
...
def points(board):
    pieces_list = [details["Piece"] for details in board.values()]
    white_pawn_count = pieces_list.count("P")
    white_rook_count = pieces_list.count("R")
    white_bishop_count = pieces_list.count("B")
    white_knight_count = pieces_list.count("N")
    white_quenn_count = pieces_list.count("Q")

    black_pawn_count = pieces_list.count("p")
    black_rook_count = pieces_list.count("r")
    black_bishop_count = pieces_list.count("b")
    black_knight_count = pieces_list.count("n")
    black_quenn_count = pieces_list.count("q")

    white_points = white_pawn_count * 1 + white_rook_count * 5 + white_bishop_count * 3 + white_knight_count * 3 + white_quenn_count * 9
    black_points = black_pawn_count * 1 + black_rook_count * 5 + black_bishop_count * 3 + black_knight_count * 3 + black_quenn_count * 9
